...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-ibdsyncmode=<verify|trust>", "Choose how Initial Block Download treats blocks covered by -assumevalid. "
                   "'verify' ignores any assumevalid checkpoint and independently verifies every script back to genesis (slower, no trust assumptions). "
                   "'trust' (this node's inspiration: libbitcoin's milestone sync) skips script verification for blocks at or below the assumevalid checkpoint and trusts that they are valid -- this is ONLY safe if you, or software you trust, already verified the chain up to that point; if unset, the existing -assumevalid default is used unchanged.",
    ...
    argsman.AddArg("-parpriority", strprintf("Ask the OS scheduler to favor script verification worker threads, which can speed up Initial Block Download (default: %u). Best-effort: has no effect on platforms/permission levels that don't allow raising thread priority.", true), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-upgradereminder", strprintf("Show a one-time, non-blocking reminder to consider upgrading after this POSIX timestamp (default: %s). Purely advisory: does not affect startup, block validation, or mining in any way.", DEFAULT_UPGRADE_REMINDER_TIME), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    g_upgrade_reminder_time = args.GetIntArg("-upgradereminder", DEFAULT_UPGRADE_REMINDER_TIME);
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (g_upgrade_reminder_time) {
        scheduler.scheduleFromNow([&node] {
            auto msg = strprintf(_("This Bit-Block build has passed its two-year mark (as of %s). It's a good idea to check for and install a newer release when convenient."), FormatISO8601Date(g_upgrade_reminder_time));
            node.chainman->GetNotifications().warningSet(kernel::Warning::UPGRADE_REMINDER, msg);
            uiInterface.ThreadSafeMessageBox(msg, "Bit-Block", CClientUIInterface::ICON_INFORMATION | CClientUIInterface::BTN_OK);
        }, std::chrono::seconds{std::max<int64_t>(1, g_upgrade_reminder_time - GetTime())});
    }
